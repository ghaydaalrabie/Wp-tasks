<?php 
add_action( 'wp_enqueue_scripts', 'advertisingly_blog_enqueue_styles' );
function advertisingly_blog_enqueue_styles() {
	wp_enqueue_style( 'advertisingly-blog-parent-style', get_template_directory_uri() . '/style.css' ); 
} 


function advertisingly_blog_google_fonts() {
	wp_enqueue_style( 'advertisingly-blog-google-fonts', '//fonts.googleapis.com/css?family=Inter:400,600,700', false ); 
}
add_action( 'wp_enqueue_scripts', 'advertisingly_blog_google_fonts' );
 ...
 */
function advertisingly_blog_customize_preview_js() {
	wp_enqueue_script( 'advertisingly-blog-customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'advertisingly_blog_customize_preview_js' );


function advertisingly_blog_customize_register( $wp_customize ) {
...
$wp_customize->add_setting( 'header_img_text', array(
	...
	$wp_customize->add_control( 'header_img_text', array(
		'label'    => __( "Title", 'advertisingly-blog' ),
	...
	$wp_customize->add_control( 'header_img_text_tagline', array(
		'label'    => __( "Tagline", 'advertisingly-blog' ),
		'section'  => 'header_image',
		'type'     => 'text',
		'priority' => 1,
		) ); 
	...
	$wp_customize->add_control( 'readmoretext', array(
		'label'    => __( "Read More Text", 'advertisingly-blog' ),
		'section'  => 'header_image',
		'type'     => 'text',
		'priority' => 1,
		) );
	...
	$wp_customize->add_control( 'readmorelink', array(
		'label'    => __( "Read More Link URL", 'advertisingly-blog' ),
		'section'  => 'header_image',
		'type'     => 'text',
		'priority' => 1,
		) );
	...
	$wp_customize->add_setting( 'advertisingly_blog_readmore_text_color', array(
		'default'           => '#fff',
		'sanitize_callback' => 'sanitize_hex_color',
		'transport'         => 'refresh',
	) );
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'advertisingly_blog_readmore_text_color', array(
		'label'       => __( 'Read More Button Text Color', 'advertisingly-blog' ),
		'section'     => 'colors',
		'priority'   => 1,
		'settings'    => 'advertisingly_blog_readmore_text_color',
	) ) );



	$wp_customize->add_setting( 'advertisingly_blog_readmore_bg_color', array(
		'default'           => '#007c89',
		'sanitize_callback' => 'sanitize_hex_color',
		'transport'         => 'refresh',
	) );
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'advertisingly_blog_readmore_bg_color', array(
		'label'       => __( 'Read More Button Background Color', 'advertisingly-blog' ),
		'section'     => 'colors',
		'priority'   => 1,
		'settings'    => 'advertisingly_blog_readmore_bg_color',
	) ) );



	function advertisingly_blog_sanitize_checkbox( $input ) {
...
add_action( 'customize_register', 'advertisingly_blog_customize_register' );




if(! function_exists('advertisingly_blog_customizer_css_final_output' ) ):
	function advertisingly_blog_customizer_css_final_output(){
		?>

		<style type="text/css">
			<?php if ( get_theme_mod( 'advertisingly_blog_hide_readmore_button' ) == '1' ) : ?>
				.read-more-blogfeed { display: none; }
			<?php endif; ?>
 .read-more-blogfeed a {background: <?php echo esc_attr(get_theme_mod( 'advertisingly_blog_readmore_bg_color')); ?>; }
 .read-more-blogfeed a {color: <?php echo esc_attr(get_theme_mod( 'advertisingly_blog_readmore_text_color')); ?>; }
	...
	<?php }
	add_action( 'wp_head', 'advertisingly_blog_customizer_css_final_output', 99999 );
endif;
